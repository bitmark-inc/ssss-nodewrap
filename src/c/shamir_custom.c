...
EXPORT error_t wrapped_split(char **shares, const char *secret, int security, int threshold, int number, bool diffusion, const char *prefix, bool hexmode, const char *random_bytes, size_t byte_count) {
	...
	char ** temp_shares = wrapped_allocate_shares(number);
	error_t error =  split(secret, internal_split_cb, temp_shares, security, threshold, number, diffusion, prefix, hexmode, NULL == random_bytes ? NULL : &buffered_cprng);
	int i;
	for (i = 0 ; i < number; i++) {
		shares[i] = malloc(strlen(temp_shares[i]));
		strcpy(shares[i], temp_shares[i]);
	}
	free(temp_shares);
	return error;
}
